...
class RAM_REST_Register_Controller extends WP_REST_Controller {
    ...
    public function __construct() {
        ...
        $this->max_duration = 5; // 注册流程最大时间，单位为分钟
    }
    ...
    function getGraphicCaptcha($request) {
        ...
        session_start(["name" => "token", "gc_maxlifetime" => strval($this->max_duration * 60), "use_cookies" => false]);
        $token = session_id();
    ...
    function resetPassword($request) {
        return rest_ensure_response(retrieve_password($request["email"]));
    }
